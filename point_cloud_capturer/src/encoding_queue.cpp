...
{   
    float scale_factor = (std::min)(1.0f, 125000.0f / pc->n_points);
    UniformSampler us(std::vector<float>({ 0.6f, 0.25f, 0.15f }));
    std::vector<Description*> descs = us.create_descriptions(pc, scale_factor);
    pc->n_points = 0;
    bool is_valid_frame = true;
    for(auto d : descs) {
        d->n_points = d->current_points;
        pc->n_points += d->n_points;
        if(d->current_points < 100) {
            is_valid_frame = false;
        }
    }
    for(auto d : descs) {
        d->n_points_in_total = pc->n_points;
    }
    free_pc_callback_instance(current_in_wait);
