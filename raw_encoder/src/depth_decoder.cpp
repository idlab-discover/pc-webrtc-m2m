...
DecodedDepth* DepthDecoder::decode_depth(unsigned char* raw_depth, unsigned int width, unsigned int height) {
...
void DepthDecoder::decompress_rvl(unsigned char *input, unsigned short *output, int numPixels)
{
    int* buffer;
    int* p_buffer;
...
int DepthDecoder::decode_vle(int*& p_buffer)
{
    unsigned int nibble;
    int value = 0, bits = 29;
    do
