...
public class CapturingTestMultiAudio : MonoBehaviour
{
    ...
    private List<AudioPlayback> audioPlaybacks = new List<AudioPlayback>();
        ...
        if (initCode == 0)
        {
            for (int i = 0; i < renderers.Count; i++)
            {
                ...
                } else
                {
                    //if(i == 0)
                   // {
                        AudioPlayback p = Instantiate(AudioPlaybackPrefab, renderers[i].transform);
                        p.Init(Capture.CaptureSrate, AudioParams);
                        audioPlaybacks.Add(p);
                  //  }
    ...
    void Update()
    {
        if (Capture.RecordingStarted)
        {
            Debug.Log("rec");
            foreach (AudioPlayback play in audioPlaybacks)
            {
                play.StartPlayback();
            }
        }
        if (!queue.IsEmpty)
        {
           ...
           //     Debug.Log($"Bounds: {currentMesh.bounds}");
                ...
                foreach (AudioPlayback play in audioPlaybacks)
                {
                    if(play.HasStarted())
                    {
                       //    play.SetTimestampLatestPC(dec.Timestamp);
                    }
